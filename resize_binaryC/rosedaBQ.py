...
import re, traceback, sys, random, time
import datetime
...
def dict2BQ(data_dict, bqClient, datasetName, tableName, BQauto=False, clean=True):
    ...
    try:
        ...
        print("assuming rate limit exceeded and trying again")
        print(str(job.errors))
        time.sleep(random.uniform(1,5))
        job = dict2BQ(data_dict, bqClient, datasetName, tableName, BQauto=BQauto, clean=clean)
    except:
        print(str(job.errors))
        traceback.print_exc(file=sys.stdout)
        return job
